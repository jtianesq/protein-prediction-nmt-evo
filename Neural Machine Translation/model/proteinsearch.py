...
def beamsearch(models, seq, mask=None, beamsize=10, normalize=False,
    ...
    for k in range(maxlen):
        ...
        num = len(candidates)
        last_words = numpy.array(list(map(lambda cand: cand[-1], candidates)), "int32")

        # compute context first, then compute word distribution
        batch_mask = numpy.repeat(mask, num, 1)
        batch_annots0 = list(map(numpy.repeat, annotations0, [num] * num_models,
                            [1] * num_models))
        batch_annots1 = list(map(numpy.repeat, annotations1, [num] * num_models,
                            [1] * num_models))
        batch_mannots0 = list(map(numpy.repeat, mapped_annots0, [num] * num_models,
                             [1] * num_models))
        batch_mannots1 = list(map(numpy.repeat, mapped_annots1, [num] * num_models,
                             [1] * num_models))
        batch_soft_mask = list(map(numpy.repeat, soft_masks, [num] * num_models, [1] * num_models))
        ...
        num = len(candidates)
        last_words = numpy.array(list(map(lambda t: t[-1], candidates)), "int32")
        # print(last_words)
    ...
    for trans, score in zip(hypo_list, score_list):
        trans = list(map(lambda x: vocab[x], trans))
...
def batchsample(model, seq, mask, maxlen=None):
    ...
    for i in range(trans.shape[1]):
        ...
        example = list(map(lambda x: vocab[x], example))
