...
class Progress:
    ...
    def add_valid(self, group, src, ref_stem, external_validation_script, main_entry, option, modelname, bestname,
        ...
        if self.delay_val:
            ...
            evaluate_score(src, ref_stem, external_validation_script, main_entry, option, self, modelname, bestname,
                          serializer)
...
class EvalTask(Task):
    ...
    def run(self, shared_dict):
        ...
        sec_type = {"G" : "helix", "H" : "helix", "I" : "helix", "T": "coil", "S" : "coil", "L" : "coil", "C": "coil", "B" : "strand", "E": "strand", "NoSeq": "none", "*EOS*" : "beg", "*SOS*" : "end"}
        ...
        actual = open(self.ref_stem, "r").readlines()[0].split()

        out_lst = out.decode("utf-8").strip().split()

        # print(out_lst, actual)

        score = scoring(out_lst, actual)
...
def evaluate_score(src, ref_stem, ext_script, main_entry, option, progress, modelname, bestname, serializer):
    tic = time.time()
    if not os.path.exists('tmp'):
        os.makedirs('tmp')
    savename = os.path.join('tmp', '{}.iter{}-{}.{}.pkl'.format(modelname, progress.epoch, progress.batch_count,
    ...
    print_eval(progress.epoch, progress.batch_count, progress, False, score, elapse)


def print_eval(epoch, batch_count, progress, delay, score, elapse):
    item = ((epoch, batch_count), score)
    progress.valid_hist.append(item)
    print('{}-{} score: {:.4f}, {}'.format(epoch + 1, batch_count, score,
